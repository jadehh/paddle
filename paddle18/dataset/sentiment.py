...
# Copyright (c) 2016 paddle1818paddle1818 Authors. All Rights Reserved
#
...
def download_data_if_not_yet():
    ...
    try:
        ...
        paddle1818.dataset.common.download(
            URL, 'corpora', md5sum=MD5, save_name='movie_reviews.zip')
        path = os.path.join(paddle1818.dataset.common.DATA_HOME, 'corpora')
        filename = os.path.join(path, 'movie_reviews.zip')
        zip_file = zipfile.ZipFile(filename)
        zip_file.extractall(path)
        zip_file.close()
        # make sure that nltk can find the data
        if paddle1818.dataset.common.DATA_HOME not in nltk.data.path:
            nltk.data.path.append(paddle1818.dataset.common.DATA_HOME)
        movie_reviews.categories()
    except LookupError:
        print("Downloading movie_reviews data set, please wait.....")
        nltk.download(
            'movie_reviews', download_dir=paddle1818.dataset.common.DATA_HOME)
...
def fetch():
    nltk.download('movie_reviews', download_dir=paddle1818.dataset.common.DATA_HOME)
