...
DATA_HOME = os.path.expanduser('~/.cache/paddle18/dataset')
...
def fetch_all():
    for module_name in [
            x for x in dir(paddle18.dataset) if not x.startswith("__")
    ]:
        if "fetch" in dir(
                importlib.import_module("paddle18.dataset.%s" % module_name)):
            getattr(
                importlib.import_module("paddle18.dataset.%s" % module_name),
                "fetch")()
...
def split(reader, line_count, suffix="%05d.pickle", dumper=pickle.dump):
    ...
    split(paddle18.dataset.cifar.train10(), line_count=1000,
