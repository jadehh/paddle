#   Copyright (c) 2018 paddle18paddle18 Authors. All Rights Reserved.
#
...
import paddle18.version as fluid_version
import warnings
...
def require_version(min_version, max_version=None):
    """
        Check if the installed version of paddle18paddle18 is in [min_version, max_version],
        ...
        Examples:
            .. code-block:: python
                ...
                import paddle18.fluid as fluid
    ...
    if version_cmp(version_installed, zero_version) == 0:
        if max_version is not None:
            warnings.warn(
                "paddle18paddle18 version in [%s, %s] required, but %s installed. "
                "Maybe you are using a develop version, "
                "please make sure the version is good with your code." %
                (min_version, max_version, fluid_version.full_version))
        else:
            warnings.warn(
                "paddle18paddle18 version %s or higher is required, but %s installed, "
                "Maybe you are using a develop version, "
                "please make sure the version is good with your code." %
    ...
    if max_version is not None:
        ...
        if version_cmp(version_installed,
            ...
            raise Exception(
                "VersionError: paddle18paddle18 version in [%s, %s] required, but %s installed."
                % (min_version, max_version, fluid_version.full_version))
    else:
        if version_cmp(version_installed, min_version_to_check) < 0:
            raise Exception(
                "VersionError: paddle18paddle18 version %s or higher is required, but %s installed, "
                "please upgrade your paddle18paddle18 to %s or other higher version."
...
def _var_base_to_np(var_base):
    ...
    warnings.warn(
        "paddle18.fluid.framework._var_base_to_np is deprecated, please use var_base.numpy() instead of _var_base_to_np(var_base)."
    )
...
def convert_np_dtype_to_dtype_(np_dtype):
    """
    Convert the data type in numpy to the data type in paddle18

    Args:
        np_dtype(np.dtype): the data type in numpy.

    Returns:
        core.VarDesc.VarType: the data type in paddle18.
...
def dtype_is_floating(dtype):
    ...
    Args:
        dtype(np.dtype|core.VarDesc.VarType): data type.
            Could be numpy format or paddle18 format
...
class Variable(object):
    ...
    **Notes**:
    ...
    and usages. Please refer to the `framework.proto <https://github.com/paddle18paddle18/paddle18/blob/develop/paddle18/fluid/framework/framework.proto>`_ for details.
        ...
        **Notes**:
        ...
        Examples:
            .. code-block:: python
                ...
                import paddle18.fluid as fluid
                from paddle18.fluid.dygraph.base import to_variable
                from paddle18.fluid.dygraph import Linear
                import numpy as np
...
def get_all_op_protos():
    """
    Get all registered op proto from paddle18paddle18 C++ end.
...
class Operator(object):
    ...
    def __init__(self,
        ...
        if in_dygraph_mode():
            ...
            # https://github.com/paddle18paddle18/paddle18/pull/12583#pullrequestreview-145093173
            op_attrs = attrs
            if op_attrs is None:
                op_attrs = dict()
            del attrs
    ...
    def idx(self):
        for i, op in enumerate(self.block.ops):
            if op == self:
                return i
        raise ValueError(
            "Can't find op itself in it's block. It could be a bug of paddle18.")
...
class Program(object):
    ...
    `framework.proto <https://github.com/paddle18paddle18/paddle18/blob/develop/paddle18/fluid/framework/framework.proto>`_
    for details.
    ...
    paddle18 will contain all content to build a train network,  in test
    program paddle18 will prune some content which is irrelevant to test, eg.
        ...
        **Notes: The Program's order maybe different after** :code:`clone` **and
        ...
        1. To clone a test program, the sample code is:
                .. code-block:: python
                    ...
                    # In paddle18 Fluid we will share weights by using the same Variable name. In train and test program
...
def default_startup_program():
    ...
    The layer function in :ref:`api_fluid_layers` will create parameters, :ref:`api_paddle18_data_reader_reader` ,
...
def load_op_library(lib_filename):
    ...
    will be available in paddle18paddle18 main process.
