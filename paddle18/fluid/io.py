...
from paddle18.reader import *
from paddle18.fluid.executor import Executor, global_scope
from paddle18.fluid.framework import Program, Parameter, default_main_program, Variable
from paddle18.fluid.compiler import CompiledProgram
from paddle18.fluid.log_helper import get_logger
from . import reader
from . import unique_name
from . import core
from .. import compat as cpt

batch = paddle18.batch
...
] + reader.__all__ + paddle18.reader.__all__
...
def _load_program_scope(main=None, startup=None, scope=None):
    prog = main if main else paddle18.fluid.Program()
    startup_prog = startup if startup else paddle18.fluid.Program()
    scope = scope if scope else paddle18.fluid.core.Scope()
    with paddle18.fluid.scope_guard(scope):
        with paddle18.fluid.program_guard(prog, startup_prog):
            with paddle18.fluid.unique_name.guard():
                yield
...
def save_params(executor, dirname, main_program=None, filename=None):
    ...
    Examples:
        .. code-block:: python
            ...
            # be saved in different files in the path "./my_paddle18_model" 
    """
    return save_vars(
        executor,
        dirname=dirname,
...
def load_vars(executor,
    ...
    Examples:
        .. code-block:: python
            ...
            # be saved in the same file named 'var_file' in the path "./my_paddle18_vars".
    ...
    if vars is None:
        ...
        for each_var in vars:
            if not isinstance(each_var, Parameter):
                continue
            var_temp = paddle18.fluid.global_scope().find_var(each_var.name)
...
def save_inference_model(dirname,
    ...
    if isinstance(feeded_var_names, six.string_types):
        ...
        if len(feeded_var_names) > 0:
            # TODO(paddle18-dev): polish these code blocks
    ...
    if export_for_deployment:
        ...
        paddle18.fluid.core.save_op_compatible_info(main_program.desc)
...
def save(program, model_path):
    ...
    program.desc.flush()
    main_program.desc._set_version()
    paddle18.fluid.core.save_op_compatible_info(program.desc)
...
def load(program, model_path, executor=None, var_list=None):
    ...
    def set_var(var, ndarray):
        ...
        p = t._place()
        if p.is_cpu_place():
            place = paddle18.fluid.CPUPlace()
        elif p.is_cuda_pinned_place():
            place = paddle18.fluid.CUDAPinnedPlace()
        else:
            p = paddle18.fluid.core.Place()
            p.set_place(t._place())
            place = paddle18.fluid.CUDAPlace(p.gpu_device_id())
    ...
    if executor:
        paddle18.fluid.core._create_loaded_parameter(parameter_list,
                                                   global_scope(),
    ...
    if len(optimizer_var_list) > 0:
        ...
        if executor:
            paddle18.fluid.core._create_loaded_parameter(
...
def load_program_state(model_path, var_list=None):
    ...
    if not os.path.exists(parameter_file_name):
        ...
        with _load_program_scope():
            ...
            place = paddle18.fluid.CPUPlace()
            exe = paddle18.fluid.Executor(place)
            ...
            for var in loaded_var_list:
                res_dict[var.name] = np.asarray(paddle18.fluid.global_scope(
...
def set_program_state(program, state_dict):
    ...
    for para in parameter_list:
        var_temp = paddle18.fluid.global_scope().find_var(para.name)
        ...
        if para.name in state_dict:
            ...
            py_place = paddle18.fluid.CPUPlace()
            if ten_place.is_cuda_pinned_place():
                place = paddle18.fluid.CUDAPinnedPlace()
            elif ten_place.is_gpu_place():
                p = paddle18.fluid.core.Place()
                p.set_place(ten_place)
                py_place = paddle18.fluid.CUDAPlace(p.gpu_device_id())
