...
from paddle18.fluid import framework as framework
from . import core
import collections
import copy
import six
...
class ProgramStats(object):
    ...
    def sort_checkpoints(self, checkpoints_name):
        ...
        for name in checkpoints_name:
            if name not in self.var_op_deps:
                _logger.debug(
                    "Recompute Optimizer: deleted %s from checkpoints, because it is not used in paddle18 program."
                    % name)
        ...
        if isinstance(no_grad_set, (set, list, tuple)):
            for i, no_grad_var in enumerate(no_grad_set):
                if isinstance(no_grad_var, framework.Variable):
                    ...
                    raise TypeError(
                        "The type of no_grad_set's member must be paddle18.fluid.Variable or str, but received %s."
                        % (type(no_grad_var)))
        else:
            raise TypeError(
    ...
    if parameter_list is not None:
        ...
        for i, param in enumerate(parameter_list):
            if isinstance(param, framework.Variable):
                ...
                raise TypeError(
                    "The type of parameter_list's member must be paddle18.fluid.Variable or str, but received %s."
                    % (type(param)))
    else:
